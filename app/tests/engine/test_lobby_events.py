...
from app.engine.lobby_events import CreateLobbyEvent, EventTypeEnum, InvalidEvent, JoinLobbyEvent, LobbyFullEvent, LobbyNotFoundEvent, LobbyStartingEvent, LobbyUser, StartLobbyEvent, StateSyncEvent, parse_lobby_event
...
def test_start_lobby():
    event = StartLobbyEvent(data={'user_id':"TEST_USER_ID"})

    assert event.type == EventTypeEnum.START_LOBBY
    assert event.data.user_id == "TEST_USER_ID"

    serialized = event.serialize_event()

    assert serialized['type'] == EventTypeEnum.START_LOBBY
    assert serialized['data']['user_id'] == "TEST_USER_ID"

def test_parse_lobby_event_success():
    event = LobbyFullEvent()
    parsed_event = parse_lobby_event(event.__dict__)
...
def test_parse_lobby_event_failure():
    event = {'type':"TEST_TYPE", 'data':"Test"}

    parsed_event = parse_lobby_event(event)
